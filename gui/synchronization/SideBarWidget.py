...
class SideBarWidget(QWidget):
    ...
    def delete_tag(self):
        temp_value = self.tags_table.selectedItems()[0].text().split(':')
        timecode = datetime(1970, 1, 1, int(temp_value[0]), int(temp_value[1]), int(temp_value[2]),
                         int(temp_value[3]) * 1000)
        self.shared_data.update_tags.emit('delete', timecode, self.tags_table.selectedItems()[1].text())
    ...
    def restore_state(self):
        if self.shared_data.video_sync is not None or self.shared_data.data_sync is not None:
